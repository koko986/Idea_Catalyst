...
import { conversationHref, draftThread, formatChatTime, lastPreview, persistConversations, sendMessage } from "@/lib/chat";

export function ChatInbox({ embedded = false }: { embedded?: boolean }) {
  ...
  const body = conversations.length === 0 ? (
    <section className="card inbox-empty">
      ...
      <p className="muted" style={{maxWidth:420,margin:"0 auto"}}>This Seller tab stays empty until you message a seller from a listing or a customer from an offer.</p>
    </section>
  ) : (
    <section className="card inbox" aria-label="Chat history">
      {conversations.map((conversation) => (
        <Link className="inbox-row" key={conversation.id} href={conversationHref(conversation)}>
  ...
  return (
    <main className="page">
      ...
      <p className="muted" style={{maxWidth:520,marginBottom:28}}>Your conversations stay on the marketplace Seller tab so you can pick them up again anytime.</p>
      {body}
    </main>
  );
}
...
export function ChatThread({
  threadId,
  embedded = false,
  backHref = "/marketplace?tab=seller",
  backLabel = "Seller chats",
}: {
  threadId: string;
  embedded?: boolean;
  backHref?: string;
  ...
  const missing = (
    <>
      {!embedded && <Link href={backHref} className="chat-back"><ChevronLeft size={18}/> {backLabel}</Link>}
      <section className="card inbox-empty">
  ...
  const thread = conversation && (
    <>
      ...
      <div className={embedded ? undefined : "split"}>
        <section className="card chat">
          <div className="messages">
            ...
            {conversation.messages.length === 0 && (
              <p className="muted" style={{textAlign:"center",padding:"28px 8px"}}>No messages yet. Say hello and this chat will stay in the Seller tab.</p>
            )}
  ...
  const content = conversation ? thread : missing;
