...
import { initials, listingThreadId } from "@/lib/chat";
...
export default async function ListingPage({
  ...
  const sellerTab = first((await searchParams).tab) === "seller";
      ...
      <div className="split">
        <div>
          ...
          <nav className="tabs" aria-label="Listing sections">
            <Link href={`/marketplace/${item.id}`} className={`tab ${sellerTab ? "" : "active"}`}>Listing</Link>
            <Link href={`/marketplace/${item.id}?tab=seller`} className={`tab ${sellerTab ? "active" : ""}`}>Seller</Link>
          </nav>
          {sellerTab ? (
            <>
              <section>
                <div className="card">
                  <div style={{display:"flex",gap:15,alignItems:"center"}}>
                    <div className="avatar" style={{width:54,height:54}}>{initials(item.seller)}</div>
                    <div>
                      <h2 style={{fontSize:28,marginBottom:5}}>{item.seller} {item.verified && <BadgeCheck size={17} style={{display:"inline",color:"var(--brand)"}}/>}</h2>
                  ...
                  <div className="grid grid-4" style={{marginTop:22}}>
                    ...
                    <div><div className="metric">{item.rating}</div><div className="stat-label">Customer rating</div></div>
                  </div>
                  <div className="actions">
                    <span className="badge"><ShieldCheck size={14}/> Identity verified</span>
                    <span className="badge"><BadgeCheck size={14}/> Power seller</span>
                    <span className="badge"><Leaf size={14}/> 4,820 kg saved</span>
                  </div>
                </div>
              </section>
              <div style={{marginTop:18}}>
                <ChatThread threadId={listingThreadId(item.id)} embedded/>
              </div>
            </>
          ) : (
            <>
        ...
        <BuyPanel price={item.price} trial={item.trial} listingId={item.id}/>
      </div>
    </main>
  );
}
